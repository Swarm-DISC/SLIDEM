# Try to find gnu scientific library GSL, including CBLAS on unix
# See 
...
if( WIN32 AND NOT CYGWIN AND NOT MSYS )
 ...
 #       message("GSL_CFLAGS=${GSL_CFLAGS}")
      ...
      # run the gsl-config program to get the libs without CBLAS
      execute_process(
        COMMAND sh "${GSL_CONFIG_EXECUTABLE}" --libs-without-cblas
        OUTPUT_VARIABLE GSL_LIBRARY
        RESULT_VARIABLE RET
        ERROR_QUIET
        )
      if( RET EQUAL 0 )
        string(STRIP "${GSL_LIBRARY}" GSL_LIBRARY )
        separate_arguments( GSL_LIBRARY )
 
        # extract linkdirs (-L) for rpath (i.e., LINK_DIRECTORIES)
        string( REGEX MATCHALL "-L[^;]+"
          GSL_LIBRARY_DIRS "${GSL_LIBRARY}" )
        string( REPLACE "-L" ""
     ...
     # run the gsl-config program to get the libs with CBLAS
      execute_process(
        COMMAND sh "${GSL_CONFIG_EXECUTABLE}" --libs
        OUTPUT_VARIABLE GSL_CBLAS_LIBRARY
        RESULT_VARIABLE RET
        ERROR_QUIET
        )
      if( RET EQUAL 0 )
        string(STRIP "${GSL_CBLAS_LIBRARY}" GSL_CBLAS_LIBRARY )
        separate_arguments( GSL_CBLAS_LIBRARY )
 
        # extract linkdirs (-L) for rpath (i.e., LINK_DIRECTORIES)
        string( REGEX MATCHALL "-L[^;]+"
          GSL_LIBRARY_DIRS "${GSL_CBLAS_LIBRARY}" )
        string( REPLACE "-L" ""
          GSL_LIBRARY_DIRS "${GSL_LIBRARY_DIRS}" )
      else( RET EQUAL 0 )
        set( GSL_CBLAS_FOUND FALSE )
      endif( RET EQUAL 0 )
